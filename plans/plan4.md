# Conductor — Implementation Plan (Revision 4 — Final)
...
- `provider`: Routing/implementation selector — Conductor may use this to route to the correct GovernanceReader implementation
- `reference_id` + `metadata`: Opaque to Conductor core — never interpreted by Conductor
- Conductor routes to the correct reader based on `provider` field, but never inspects provider-specific payload
...
| **accepted** | Task's required work AND any required governed consequence have both completed successfully | Yes |
...
- Governance system separately approves **the consequence** (when applicable)
- `accepted` means both have completed successfully
...
| review | accepted | Reviewer accepts work AND governed consequence completed (when required) | human, agent | task.accepted |
...
- **Purpose:** Reviewer accepts completed work (and governed consequence, when required)
- **Actor:** Human or reviewer agent
- **Input:** task_id
- **Output:** Updated task
- **State mutations:** Task.status = 'accepted'
- **Activity event:** task.accepted
- **Governance interaction:** None (governance check happens externally before acceptance)

**Acceptance semantics:**
- Reviewer approves **the work**
- Governance system separately approves **the consequence** (when applicable)
- `accepted` means both have completed successfully
- Conductor records the acceptance event
...
7. Task moves to `accepted` when work AND governed consequence (when required) both complete
...
6. Reviewer accepts/rejects
   ...
   → If accepted: proceed to governed consequence (if required)

7. If governed consequence required
   → Query governance status via GovernanceReader (read-only)
   → Agent executes consequence via external governance system (not via Conductor)
   → Record outcome reference in activity

8. Task accepted
   → Task's required work AND any required governed consequence have both completed
   → Task moves to accepted (terminal)
```
...
| **Accepted semantics** | Work AND governed consequence both completed | Work only | Coherent workflow |
...
## Corrections Made (Revision 4)

### 1. GovernanceReader vs External Governance System

**Corrected:** Resolved naming ambiguity between internal interface and external system.

**Before:** `GovernanceProvider` used for both internal Go interface and external governance system.

**After:** Clear distinction:
- `GovernanceReader` = internal read-only Go interface inside Conductor
- `SolventAdapter implements GovernanceReader` = Conductor's implementation
- External governance system = separate entity (Solvent, etc.) that owns authorization and execution

**Changed:**
- Renamed `GovernanceProvider` to `GovernanceReader` throughout
- Renamed `provider.go` to `reader.go`
- Renamed `null_provider.go` to `null_reader.go`
- Added explicit distinction table: GovernanceReader vs External Governance System
...
**Corrected:** Resolved inconsistency where `accepted` was terminal but workflow performed additional work afterward.

**Before:** `accepted` meant "work complete" but governed consequences happened after acceptance.

**After:** `accepted` means "task's required work AND any required governed consequence have both completed successfully."

**Changed:**
- Updated status definition for `accepted`
- Updated lifecycle description to show governed consequence before acceptance
- Updated workflow to show governed consequence before `accepted`
- Updated acceptance semantics in API/MCP operations
...
- Accepted means work AND governed consequence both completed
