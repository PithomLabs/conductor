# Conductor — Implementation Plan (Revision 5 — Final)
...
## Corrections Made (Revision 5)

### 1. Accepted State Ownership

**Corrected:** Conductor cannot authoritatively verify external governed execution because it does not broker or perform that execution.

**Before:** `accepted` = work AND governed consequence both completed

**After:** `accepted` = the task's work has been reviewed and accepted by the designated reviewer

**Changed:**
- Updated status definition for `accepted`
- Updated accepted semantics description
- Updated lifecycle description (removed governed consequence from accepted meaning)
- Updated accept_task semantics
- Updated workflow description
- Updated acceptance criteria
- Updated invariant tests

**Key principle:** Conductor makes authoritative claims only about facts it controls. External governed consequence state is owned by the external governance system.

### 2. Governance_ref Semantics

**Corrected:** Conductor uses `provider` for routing, so governance_ref is not completely opaque.

**Before:** Conductor only knows whether governance_ref exists

**After:** Conductor interprets `provider` for routing; `reference_id` and `metadata` are opaque to Conductor core

**Changed:**
- Updated governance_ref semantics description
- Added precise abstraction diagram
- Updated invariant #6
- Updated domain-agnosticity invariant
- Updated invariant tests

**Precise statement:**
- `provider` = routing selector understood by Conductor
- `provider-specific reference payload` = opaque to Conductor core
- Conductor must never interpret reference_id, metadata, domain semantics, authorization semantics, or consequence semantics

### 3. External Execution Activity

**Corrected:** Activity records about external execution are observational/project metadata, not authoritative evidence.

**Before:** Activity records events (not explicitly constrained for external execution)

**After:** Activity records describing external execution are agent-reported/project-level observations, never authoritative evidence of execution success

**Changed:**
- Updated Activity domain model with authority constraint
- Updated Activity semantics in schema
- Added external execution activity constraint to consequential action boundary
- Updated security/trust section
- Updated end-to-end workflow
- Updated invariant tests

**Key insight:** Conductor's activity history is secondary reporting for external consequences. It should not be treated as proof of execution.

### 4. Responsibility Non-Overlap Supreme Invariant

**Added:** Every authoritative fact belongs to exactly one system.

**Changed:**
- Added Supreme Invariant section at top of document
- Added responsibility table
- Added forbidden overlaps list
- Added implementation rule
- Added invariant #12
- Added to invariant tests
- Added to Final ADR
- Added to Non-Goals

**Key principle:** Neither Conductor nor Solvent may create a competing authoritative representation of facts owned by the other.
