# Conductor — Implementation Plan (Revision 6 — Final)

## Architecture Verdict

**GREEN — APPROVED FOR IMPLEMENTATION**
...
## Supreme Invariant: Responsibility Non-Overlap
...
| **Solvent** | Governance, authority, authorization, consequential execution |
| **Domain Application** | Domain-specific trust semantics |

**Neither system may create a competing authoritative representation of facts owned by the other.**
...
Even a cached Solvent value must be a **non-authoritative projection**, never a second copy of Solvent state.
...
- **Generic governance boundary**: Solvent is one external governance system
- **Conductor is read-only for governance**: never brokers, performs, or persists consequential execution
- **GovernanceReader**: internal read-only interface, distinct from external governance systems
- **Lifecycle bypass prevention**: generic update operations cannot mutate status or current_agent
- **Atomic state + activity**: lifecycle transitions and activity events commit in one SQLite transaction
- **Same-project dependencies**: dependencies are restricted to same project for v1
...
│   ├── governance/
│   │   ├── reader.go                  # GovernanceReader interface
│   │   ├── types.go                   # Generic governance types
│   │   └── null_reader.go             # No-op implementation
│   ├── adapter/
│   │   └── solvent/
│   │       ├── adapter.go             # Solvent GovernanceReader impl
...
| **GovernanceReader** | Read-only queries to external governance system | Interface in Conductor |
| **External Governance System** | Authorizes and executes governed consequences | External (Solvent, etc.) |
| **Domain Application** | Supplies domain semantics and policy | External |

**Critical:** These are responsibility boundaries, not five separate services. Conductor is ONE service with internal boundaries.
...
Conductor:
- Queries governance state via GovernanceReader (read-only)
...
### Task Field Mutability

| Field | Mutability | Mutated By |
|-------|------------|------------|
| `title` | Mutable via `update_task` | Generic PATCH |
| `description` | Mutable via `update_task` | Generic PATCH |
| `priority` | Mutable via `update_task` | Generic PATCH |
| `status` | **Immutable via `update_task`** | Lifecycle operations only |
| `current_agent` | **Immutable via `update_task`** | `claim_task` / `release_task` only |
| `governance_ref` | Mutable via `update_task` | Generic PATCH (set at creation, rarely changed) |

**Lifecycle fields (`status`, `current_agent`) may only change through dedicated lifecycle operations.**
...
Conductor activity concerning external execution is observational/project metadata and is never authoritative evidence of execution success. Only the external governance system owns the authoritative execution outcome.
...
| **Type** | Durable state |
| **Scope** | Same-project only (task A and task B must belong to the same project) |
...
**governance_ref is partially interpretable:**
...
- `provider`: Routing/implementation selector — Conductor uses this to route to the correct GovernanceReader implementation
...
```text
governance_ref
 ├── provider → Conductor interprets for routing to correct GovernanceReader
 └── payload (reference_id + metadata) → opaque to Conductor core
```
...
**External execution activity constraint:**

> Activity records describing external governed execution are project-level observations/references supplied by an actor. They are NOT authoritative evidence that external execution succeeded. Only the external governance system owns the authoritative execution outcome.
...
**Same-project constraint:** Enforced at repository/service layer. Task A and task B must belong to the same project. Cross-project dependencies are rejected.

### Invariants

1. **Single source of truth:** Conductor owns project/task/activity/dependency state. External governance system owns governance state.
...
5. **Governance projection is computed:** Never stored. Always derived from GovernanceReader queries.
6. **governance_ref routing vs opaque:** Conductor interprets provider for routing; reference_id and metadata are opaque to Conductor core.
...
9. **Domain-agnosticity:** Conductor only knows whether governance_ref exists and which registered reader should handle it. Does not decide what is consequential, which actions require governance, or what domain policy applies.
10. **GovernanceReader is distinct from external governance system:** GovernanceReader is an internal read-only interface; external governance system is a separate entity that owns authorization and execution.
11. **External execution activity is observational:** Activity records about external execution are project-level observations, never authoritative evidence.
12. **Responsibility non-overlap:** Every authoritative fact belongs to exactly one system. Neither Conductor nor Solvent may create competing authoritative representations.
13. **Lifecycle bypass prevention:** Generic `update_task`/PATCH cannot mutate `status` or `current_agent`. These fields change only through dedicated lifecycle operations.
14. **Atomic state + activity:** Every lifecycle transition that produces an Activity event commits both changes in one SQLite transaction. Any failure rolls back both.
15. **Same-project dependencies:** Dependencies are restricted to tasks within the same project for v1.
...
| **accepted** | Task's work has been reviewed and accepted by the designated reviewer | Yes |
...
- External governed consequence remains externally owned
- `accepted` means the work represented by the task has been reviewed and accepted
- Conductor is NOT responsible for verifying external governed execution completion
- External governed consequence state is owned by the external governance system
...
| review | accepted | Reviewer accepts work | human, agent | task.accepted |
...
### Atomic State + Activity Transaction

Every lifecycle transition that produces an Activity event MUST commit both changes in one SQLite transaction:

```sql
BEGIN;
  -- Validate transition
  SELECT status FROM conductor_task WHERE id = ? AND status = ?;
  
  -- Update task state
  UPDATE conductor_task
  SET status = ?, current_agent = ?, updated_at = datetime('now')
  WHERE id = ?;
...
Any failure rolls back both. Conductor cannot commit a state transition without its corresponding activity event, or an activity event without the state transition.
...
Verify exactly one row changed in the UPDATE. If zero rows changed, the claim failed (another agent claimed it or task is not in proposed state), and the transaction is rolled back.
...
- When agent performs consequential action through Solvent, Solvent principal is created separately at external governance boundary
...
| update_task | PATCH | /v1/tasks/:id | Update task attributes (NOT status/current_agent) |
...
**Note:** There is NO API for executing governed consequences. Execution happens via external governance system directly, not through Conductor.
...
conductor_update_task       -- Update task attributes (NOT status/current_agent)
...
**Note:** There is NO MCP tool for executing governed consequences. Execution happens via external governance system directly, not through Conductor.

### Operation Details

#### update_task

- **Purpose:** Update ordinary task attributes (title, description, priority, governance_ref)
- **Actor:** Assigned agent, human
- **Input:** task_id, mutable fields
- **Output:** Updated task
- **State mutations:** title, description, priority, governance_ref (NOT status, NOT current_agent)
- **Activity event:** task.updated
- **Governance interaction:** None
- **Lifecycle bypass prevention:** This operation CANNOT mutate `status` or `current_agent`. These fields change only through dedicated lifecycle operations.
...
- **Atomicity:** State transition + activity event commit in one SQLite transaction
...
- **Activity event:** None
- **Governance interaction:** Read-only query via GovernanceReader
- **Critical:** This is purely informational. It does NOT constitute authorization. It does NOT enable execution.

---

## 7. GovernanceReader Interface

### Generic Interface (Internal, Read-Only)
...
type GovernanceReader interface {
    ...
    // external governance system. Its result is never persisted as Conductor truth
...
func (r *NullReader) CheckAuthorization(ctx context.Context, ref GovernanceReference, action string) (*AuthorizationResult, error) {
    return &AuthorizationResult{
        Allowed:   false,
        Reason:    "no external governance system configured",
        Providers: []string{},
    }, nil
}
```
...
```go
// SolventAdapter implements GovernanceReader for Solvent.
...
func (a *SolventAdapter) CheckAuthorization(ctx context.Context, ref GovernanceReference, action string) (*AuthorizationResult, error) {
    ...
    // Execution happens via external governance system directly, not through Conductor.
}
```
...
### Distinction: GovernanceReader vs External Governance System

| Concept | What It Is | Owner |
|---------|-----------|-------|
| **GovernanceReader** | Internal read-only Go interface inside Conductor | Conductor |
| **SolventAdapter** | Implementation of GovernanceReader for Solvent | Conductor |
| **External Governance System** | Actual system that authorizes and executes governed consequences | External (Solvent, etc.) |
...
Conductor owns `GovernanceReader` (internal interface).
External governance system (Solvent) owns authorization and execution.
Agent contacts external governance system directly for consequential execution.
Conductor is never part of the execution path.
...
From external governance system (via GovernanceReader):
...
| **ready** | Governance satisfied | Task can proceed to governed consequence |
...
Conductor displays governance state for situational awareness. It never uses governance state to permit or deny actions. Authorization is the external governance system's responsibility.
...
> GovernanceReader is distinct from external governance system.

GovernanceReader is an internal read-only interface. External governance system is a separate entity that owns authorization and execution.
...
External governance system determines what is permitted
    ↓
Agent executes consequential action via external governance system (not via Conductor)
```
...
**External Governance System DOES:**
...
3. Conductor queries GovernanceReader for status (read-only)
4. Agent queries external governance system for authorization (informational)
5. Agent executes consequential action via external governance system (not via Conductor)
6. Conductor records activity event (reference to execution, not execution itself)
7. Task moves to `accepted` when work reviewed and accepted (external consequence remains externally owned)
...
Conductor does NOT broker execution. That happens via external governance system directly.

### External Execution Activity Authority
...
│                                                                   │
│ NOTE: Consequences executed via external governance system,       │
│       not through Conductor                                       │
│                                                                   │
│ ACTIVITY LOG (recent)                                             │
│                                                                   │
...
- Executing governed consequences (that happens via external governance system)
...
6. Reviewer accepts/rejects
   → If rejected: task returns to active for rework
   → If accepted: task moves to accepted (terminal)
   → External governed consequence remains externally owned

7. If governed consequence required (external to Conductor)
   → Agent executes consequence via external governance system (not via Conductor)
   → Record outcome reference in activity (observational, not authoritative)
```
...
- External governance system evaluates and executes consequences
...
| Conductor loses connection to governance | Conductor continues | Governance projection shows "unknown" | Governance system continues | Conductor reconnects | Governance panel shows stale |
| Governance state changes while agent works | External governance system is authoritative | Governance projection shows current | New state | Agent re-reads governance | Governance panel updates |
| Governance becomes stale | Governance system timestamps | Governance projection shows stale | No change | Agent re-reads before consequential action | Governance panel shows stale |
| Governance unavailable | External governance system | Governance projection shows "unknown" | No change | Agent receives degraded status | Governance panel shows unknown |
| Execution fails | External governance system | Task remains in current state | Execution failed | Agent retries or reports blocker | Task blocked with error |
| Execution result ambiguous | External governance system | Task remains in current state | Execution ambiguous | Agent re-checks status | Task shown as current state |
...
| State transition without activity | SQLite transaction rolls back | No change | No change | Operation fails cleanly | No inconsistent state |
| Activity without state transition | SQLite transaction rolls back | No change | No change | Operation fails cleanly | No false activity |
...
| Test Type | SQLite | GovernanceReader | Agent |
|-----------|--------|------------------|-------|
...
- **Integration tests:** Full workflow with real SQLite and stub GovernanceReader
...
4. No duplicate truth between project layer and external governance system
...
8. governance_ref provider is used for routing; reference_id and metadata are opaque to Conductor core
...
11. Conductor only knows whether governance_ref exists and which registered reader should handle it
12. GovernanceReader is distinct from external governance system (internal interface vs external entity)
13. Activity records about external execution are observational, not authoritative evidence
14. Responsibility non-overlap: no competing authoritative representations between Conductor and Solvent
15. Lifecycle bypass prevention: generic update_task cannot mutate status or current_agent
16. Atomic state + activity: state transition and activity event commit in one SQLite transaction
17. Same-project dependencies: dependencies are restricted to tasks within the same project
...
- Repository compiles
- No kernel imports
- GovernanceReader interface defined
...
- Atomic task claiming
- Atomic state + activity transactions
...
- Lifecycle bypass prevention (generic update cannot mutate status/current_agent)
...
- Atomic state + activity transactions work correctly
- Same-project dependencies enforced
- Lifecycle bypass prevented
...
- Authentication
- Actor identity derivation
- Lifecycle bypass prevention via API
...
- Lifecycle bypass prevented via API
...
### Phase 3: GovernanceReader Integration

**Objective:** Conductor can read governance state via internal GovernanceReader interface.

**Scope:**
- NullReader implementation
- SolventAdapter implementation
- Governance service layer
- Freshness/staleness semantics

**Files/Packages Introduced:**
- `internal/governance/null_reader.go`
...
**Interfaces Introduced:**
- `NullReader` (default no-op)
- `SolventAdapter` (Solvent implementation of GovernanceReader)
...
**Acceptance Criteria:**
- NullReader returns unknown state
...
| GovernanceReader distinct from external system | ✅ Internal interface vs external entity |
| Activity about external execution is observational | ✅ Not authoritative evidence |
| Responsibility non-overlap enforced | ✅ No competing authoritative representations |
| Lifecycle bypass prevented | ✅ Generic update cannot mutate status/current_agent |
| Atomic state + activity | ✅ State transition and activity event commit atomically |
| Same-project dependencies | ✅ Dependencies restricted to same project |
...
4. **Configure GovernanceReader** implementation that understands external governance system
...
- Activity logging
- Dependency tracking
- Generic governance reading (read-only)
- Domain-neutral UI
...
| Conductor → External Governance System | Read-only queries via GovernanceReader, never writes, never brokers execution |
| External Governance System → Conductor | Returns state, never commands |
| Agent → External Governance System | Direct execution, not via Conductor |
| External Governance System → Solvent | Solvent is one implementation of external governance |

### Authentication

- **Human:** API key in HTTP header
- **Agent:** API key or MCP trusted local process
- **Conductor → External Governance System:** API key or local process
...
- External governance system manages authority and governance
- Conductor never decides "this is authorized"
- External governance system never decides "this task is complete"
...
- **GovernanceReader:** Internal read-only interface for querying external governance
- **External Governance System:** Authority, governance, consequential execution
- **Agent:** Performing work, interacting with external systems, executing via external governance system
...
- Confusing GovernanceReader with external governance system
- Creating competing authoritative representations with external governance system
...
| **Governance boundary** | Internal GovernanceReader + external governance system | GovernanceProvider (ambiguous) | Clear distinction between internal interface and external entity |
...
| **Governance execution** | Read-only Conductor, direct execution via external system | Conductor brokers execution | Cleanest boundary |
| **Task lifecycle** | PROPOSED → ACTIVE → REVIEW → ACCEPTED | PROPOSED → ACTIVE → COMPLETED | Clear review semantics |
| **Accepted semantics** | Work reviewed and accepted; external consequence remains external | Work AND governed consequence both completed | Honest ownership boundary |
...
| **Execution boundary** | Agent + external governance system | Conductor executes | Clear responsibility |
| **Testing strategy** | Real SQLite + stub GovernanceReader | Real governance for all | Deterministic tests |
...
| **Responsibility boundary** | Non-overlap supreme invariant | Shared responsibility | No competing authoritative representations |
| **Lifecycle bypass prevention** | Generic update cannot mutate status/current_agent | Generic PATCH can mutate any field | Lifecycle invariant enforced |
| **Atomic state + activity** | State transition + activity event in one transaction | Separate commits | History integrity preserved |
| **Dependency scope** | Same-project only (v1) | Cross-project allowed | Project boundary coherent |
...
2. **Authority decisions** — governed by external governance system, not Conductor
...
8. **Execution brokering** — happens via external governance system, not through Conductor
9. **Authorization persistence** — CheckAuthorization result never stored as Conductor truth
10. **External governance system ownership** — Conductor owns GovernanceReader (internal interface), not the external system itself
11. **External execution verification** — Conductor cannot authoritatively verify external execution; activity records are observational
12. **Competing authoritative representations** — Neither system may create a competing authoritative representation of facts owned by the other

These are ALL responsibilities that belong in external governance systems, domain applications, or external systems. Conductor coordinates work and observes governance; the external governance system owns authorization and consequential execution.

---

## Corrections Made (Revision 6)

### 1. Lifecycle Bypass Prevention

**Corrected:** Generic `update_task`/PATCH cannot mutate `status` or `current_agent`.

**Before:** `update_task` could potentially mutate any task field, including `status` and `current_agent`.

**After:** `update_task` may only mutate: `title`, `description`, `priority`, `governance_ref`. Lifecycle fields (`status`, `current_agent`) change only through dedicated lifecycle operations.

**Changed:**
- Added Task Field Mutability table to domain model
- Updated `update_task` API/MCP semantics
- Updated invariant tests
- Added invariant #13

**Key principle:** Lifecycle bypass prevention ensures the API cannot circumvent the task state machine.

### 2. Atomic State + Activity

**Corrected:** State transition and activity event must commit in one SQLite transaction.

**Before:** State transition and activity event could commit separately, leading to inconsistent state.

**After:** Every lifecycle transition that produces an Activity event commits both changes in one SQLite transaction. Any failure rolls back both.

**Changed:**
- Added Atomic State + Activity Transaction section
- Updated lifecycle operations to show atomicity
- Updated invariant tests
- Added invariant #14
- Updated failure table

**Key principle:** History integrity preserved — Conductor cannot commit a state transition without its corresponding activity event, or an activity event without the state transition.

### 3. Task Dependency Scope

**Corrected:** Dependencies are restricted to same-project for v1.

**Before:** Dependencies could potentially cross project boundaries.

**After:** Dependencies are restricted to tasks within the same project. Cross-project dependencies are rejected.

**Changed:**
- Added same-project constraint to dependency schema
- Updated domain model
- Updated invariant tests
- Added invariant #15

**Key principle:** Project boundary remains coherent — dependencies do not create cross-project coupling.
...
- GovernanceReader matches projection semantics
...
- Accepted state ownership is honest (Conductor owns work acceptance, not governance outcome)
- Governance_ref is precisely defined (routing vs opaque)
- External execution activity is observational (not authoritative)
- Responsibility non-overlap enforced (no competing authoritative representations)
- Lifecycle bypass prevented (generic update cannot mutate status/current_agent)
- Atomic state + activity enforced (state transition + activity event commit atomically)
- Same-project dependencies enforced (dependencies restricted to same project)
...
- Internal GovernanceReader (read-only interface) with SolventAdapter implementation
- External governance system (Solvent) owns authorization and execution
- Conductor is strictly read-only for governance (never brokers execution)
- governance_ref interpreted for routing (provider), opaque for payload (reference_id + metadata)
...
- Accepted means work reviewed and accepted; external consequence remains external
- Domain-agnostic core (Conductor only knows governance_ref exists and which reader handles it)
- One service with internal boundaries (not five services)
- Activity about external execution is observational, never authoritative
- Responsibility non-overlap supreme invariant enforced
- Lifecycle bypass prevention enforced (generic update cannot mutate status/current_agent)
- Atomic state + activity enforced (state transition + activity event commit atomically)
- Same-project dependencies enforced (dependencies restricted to same project)
...
**The key architectural sentence is preserved:**
